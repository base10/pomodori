...
describe "Pomodori::Pausa" do
  ...
  describe "saving" do
    it "saves a valid object" do
      pausa = build(:pausa, config: @config)
    ...
    it "expects a summary" do
      pausa = build(:pausa, config: @config, summary: '')
      expect(pausa.valid?).to be_false

      pausa = build(:pausa, config: @config, summary: nil)
      expect(pausa.valid?).to be_false
    end

    it "expects a duration" do
      pausa = build(:pausa, config: @config, duration: '')
      expect(pausa.valid?).to be_false
    end
    ...
    it "expects a state" do
      pausa = build(:pausa, config: @config, state: '')
      expect(pausa.valid?).to be_false
      
      pausa = build(:pausa, config: @config, state: nil)
      expect(pausa.valid?).to be_false
    end
